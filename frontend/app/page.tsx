    ...
    <main className="flex min-h-screen flex-col items-center justify-center px-4 py-16">
      ...
      <div className="mb-12 text-center">
        ...
        <h1 className="text-4xl font-bold tracking-tight text-white">
          ...
          <span className="text-green-400">Google Maps</span>
        </h1>
        <p className="mt-3 text-gray-400">Ingresá el tipo de negocio y la zona, y descargá los resultados en CSV.</p>
      </div>
      ...
      <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-white/5 p-8 backdrop-blur">
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="mb-1.5 block text-sm font-medium text-gray-300">Tipo de negocio</label>
            <input
              value={businessType}
              ...
              className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white placeholder-gray-500 outline-none ring-green-500/50 transition focus:border-green-500/50 focus:ring-2 disabled:opacity-50"
            />
          </div>

          <div>
            <label className="mb-1.5 block text-sm font-medium text-gray-300">Zona</label>
            <input
              value={zone}
          ...
          <div>
            <div className="mb-1.5 flex items-center justify-between">
              <label className="text-sm font-medium text-gray-300">Cantidad de resultados</label>
          ...
          <button
            type="submit"
            disabled={job?.status === "running"}
            className="w-full rounded-xl bg-green-500 py-3 text-sm font-semibold text-black transition hover:bg-green-400 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
          >
        ...
        {job?.status === "running" && (
          <div className="mt-6 space-y-3">
            ...
            <div className="flex items-center justify-between text-sm">
              <span className="max-w-[70%] truncate text-gray-400">{job.current_name}</span>
        ...
        {job?.status === "done" && (
          <div className="mt-6 space-y-3">
            ...
            <div className="flex items-center justify-between rounded-xl border border-green-500/20 bg-green-500/10 px-4 py-3">
              <div>
                <p className="text-sm font-medium text-green-400">¡Listo!</p>
                <p className="text-xs text-gray-400">{job.total} resultados encontrados</p>
              </div>
              <button
                onClick={handleDownload}
                className="flex items-center gap-2 rounded-lg bg-green-500 px-4 py-2 text-sm font-semibold text-black transition hover:bg-green-400 active:scale-95"
              >
        ...
        {job?.status === "error" && (
          <div className="mt-6 rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-3">
            ...
            <p className="mt-0.5 text-xs text-gray-400">{job.error}</p>
          </div>
        )}
      </div>
